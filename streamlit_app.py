...
def monitoring_trading():
    ...
    if st.button("Calculate The Profit or Loss"):
       ...
       # st.write(f"Trading Date: {processing_date.strftime('%d-%m-%Y')}, Processing Time: {processing_time}")
        ...
        # st.write(f"Purchase Price per Share: {purchase_price_per_share} "
                # f"GBX  - Purchase Time: {purchase_time}")
       # st.write(f"Selling Price per Share: {selling_price_per_share} "
                 f"GBX  - Selling Time: {selling_time}")
